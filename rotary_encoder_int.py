...
class RotaryEncoder:
    ...
    def update(self, gpio_state):
        ...
        if current_clk != self.last_clk and current_clk == 0:
            if current_dt != current_clk:
                direction = 1
                # logger.info(f"{encoder['name']} turned {direction}, send to {encoder['cc']}")
            else:
                direction = -1
            cc_value = self.increment_cc_value(direction)
            logger.debug(f"{self.name} [CC{self.cc}]: {cc_value}")
            changed = True
...
if __name__ == "__main__":
    ...
    int_pins = {"mcp1": digitalio.DigitalInOut(board.D22), # Pin 7 Pisound
                "mcp2": digitalio.DigitalInOut(board.D5)}  # Pin 5 Pisound
    for pin in int_pins.values():
    ...
    encoders_mcp1 = [
        RotaryEncoder(midi_out, mcp1, "Rotary 1", 0, 8, 9, ENCODER_CC_NUMBERS[0]),
        RotaryEncoder(midi_out, mcp1, "Rotary 2", 3, 2, 1, ENCODER_CC_NUMBERS[1])
    ]

    encoders_mcp2 = [
        RotaryEncoder(midi_out, mcp2, "Rotary 3", 15, 14, 13, ENCODER_CC_NUMBERS[2]),
        RotaryEncoder(midi_out, mcp2, "Rotary 4", 12, 11, 10, ENCODER_CC_NUMBERS[3])
    ]

    def watchdog_thread():
        while True:
            if not int_pins["mcp1"].value:
                data = mcp1.gpio
                for enc in encoders_mcp1:
                    enc.update(data)

            if not int_pins["mcp2"].value:
                data = mcp2.gpio
